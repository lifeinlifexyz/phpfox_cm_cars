 ...
 */
class Cars_Service_Custom_Custom extends Phpfox_Service
{
	private $_sAlias = 'cf_';
	...
	{
		$this->_sTable = Phpfox::getT('cars_custom_field');
	}
	...
	public function getUserCustomValue($iUserId, $sFieldName)
	{
		return $this->database()->select($sFieldName)
			->from(Phpfox::getT('cars_custom_value'))
			->where('car_id = ' . (int) $iUserId)
	 ...
	 *
	 * `user_field`.`relation_with` == 1 ? `user_field`.`car_id` = `custom_relation_data`.`with_car_id`
	 *
	 * @param type $aRelation Can have a feed but always has the user whose profile we're looking at as car_id
	 * @param type $aReplace
	 */
//	public function getRelationshipPhrase($aRelation, $aFeed = array(), $aReplace = array(), $sPrevious = '')
//	{
//	    // $sCacheName = 'relationship_phrase_' . $aRelation['car_id'] . '_' . $aRelation['relation_data_id'] . (isset($aRelation['is_header']) && $aRelation['is_header'] == true ? '_header' : '');
...
//		}*/
//		$bMustComplete = false;
//		if (!isset($aRelation['with_car_id']))
//		{
//			$bMustComplete = true;
//			if (!isset($aRelation['relation_data_id']))
//			{
//				return Phpfox_Error::set('aRelation does not have relation_data_id or with_car_id');
//			}
//
//			$sCacheId = $this->cache()->set(array('relations', $aRelation['car_id']));
...
//					$aUser = $this->database()->select('cr.relation_id, crd.relation_data_id, crd.status_id, u.user_name, u.full_name, u.car_id, cr.phrase_var_name, cr.confirmation')
//						->from(Phpfox::getT('custom_relation_data'), 'crd')
//						->join(Phpfox::getT('custom_relation'), 'cr', 'cr.relation_id = crd.relation_id')
//						->leftjoin(Phpfox::getT('user'), 'u', 'u.car_id = crd.car_id')
...
//						->from(Phpfox::getT('custom_relation_data'), 'crd')
//						->join(Phpfox::getT('custom_relation'), 'cr', 'cr.relation_id = crd.relation_id')
//						->leftjoin(Phpfox::getT('user'), 'u', 'u.car_id = crd.with_car_id')
//						->where('crd.relation_data_id = ' . $aRelation['relation_data_id'] .' ')
//						->execute('getSlaveRow');
//				}
//
...
//				&& (
//				(!isset($aUser['car_id']) || empty($aUser['car_id']))
//				|| ($aUser['status_id'] != 2)
...
//				$aRelation['with_car_id'] = $aUser['car_id'];
...
//			$aReplace['with_car_id'] = $aRelation['with_car_id'];
//
//		}
//
//
...
//		{
//			$aLastRelation = Phpfox::getService('custom.relation')->getLatestForUser($aRelation['car_id'], $aRelation['relation_data_id']);
//
...
//			}
//			if (isset($aLastRelation['with_user']['car_id']))
//			{
//				$aReplace['with_car_id'] = $aLastRelation['with_user']['car_id'];
//			}
//		}
//
...
//		}
//
//		if (isset($aFeed['car_id']) && $aRelation['with_car_id'] == $aFeed['car_id'])
//		{
...
//		}
//		elseif ($aRelation['status_id'] == 2 && $aRelation['with_car_id'] == 0 && $sPrevious != '' && $sPrevious != 'custom.custom_relation_blank')
//		{
//			/* User went from one relationship to another */
//			$sPhrase = 'custom.relation_went_from_old_status_to_new_status';
//		}
//		elseif ($aRelation['status_id'] == 1 && $aRelation['car_id'] != $aRelation['with_car_id'])
//		{
//			/* user is in a relation that has not been confirmed yet */
//			$sPhrase = $aRelation['phrase_var_name'] . '_new';
//		}
//		elseif ($aRelation['status_id'] == 2 && (isset($aRelation['with_car_id']) && $aRelation['with_car_id'] != 0 || isset($aRelplace['with_car_id']) && $aReplace['with_car_id'] != 0))
//		{
...
//		if (isset($aRelation['car_id']) && $aRelation['car_id'] == Phpfox::getUserId())
//		{
...
//	}
	...
	public function searchFields($aParams)
	{
		...
		if (!($aFields = $this->cache()->get($sCacheId)))
		{
			$aRows = $this->database()->select('field_id, field_name, var_type')
				->from(Phpfox::getT('cars_custom_field'))
				->execute('getRows');
		...
		foreach ($aParams as $iKey => $mValue)
		{
				...
				)
			{
				...
				if ($mTest == 0 || !is_numeric($mTest)) // "any"
				{
					...
					$aOptions = $this->database()->select('option_id')
						->from(Phpfox::getT('cars_custom_option'))
					...
					$mValue = rtrim($mValue, ',') . ')';
					$aConds[] = array(
					    'table' => Phpfox::getT('cars_custom_multiple_value'),
					    'alias' => 'mvc',
					    'on' => 'mvc.car_id = c.car_id',
			...
			else
			{
				...
				$aConds[] = array(
					'table' => Phpfox::getT('cars_custom'),
					'alias' => $sAlias,
					'on' => $sAlias . '.car_id = c.car_id',
					'where' => $sAlias . '.cf_' . $aFields[$iKey]['field_name'] . ' LIKE \'%' . $this->database()->escape($mValue) . '%\''
				);
				// $aConds[] = 'ucv.cf_' . $aFields[$iKey]['field_name'] . ' LIKE \'%' . $this->database()->escape($mValue) . '%\' AND ucv.car_id = u.car_id';
			}
		}
	...
	public function getForPublic($sType, $iUserGroup = 0)
	{
		...
		if (!($aCustom = $this->cache()->get($sCacheId)))
		{
			$aGroups = $this->database()->select('group_id, phrase_var_name')
				->from(Phpfox::getT('cars_custom_group'))
				->where('type_id = \'' . $this->database()->escape($sType) . '\' ' . ($iUserGroup > 0 ? 'AND user_group_id = ' . (int) $iUserGroup : '') . ' AND is_active = 1')
				->execute('getRows');

			foreach ($aGroups as $aGroup)
			{
				$aFields = $this->database()->select('field_id, field_name, phrase_var_name, var_type, is_required')
					->from(Phpfox::getT('cars_custom_field'))
    ...
    public function getFields($sType){
        $sCacheId = $this->cache()->set('cars_custom_public_fields_' . md5($sType));

        $aFields = array();
        if (!($aFields = $this->cache()->get($sCacheId)))
        {
            $aGroups = $this->getForPublic($sType);
			if  (is_array($aGroups)){
				foreach ($aGroups as $aGroup)
				{
         ...
         * @param type $sTypeId
         * @param type $iItemId most often (or always) its the car_id of the profile (can be != Phpfox::getUserId())
	...
	public function getForDisplay($sTypeId, $iItemId, $iUserGroupId = null)
	{
		...
		if ((!($aFields = $this->cache()->get($sCacheId))) || (!is_array($aFields)))
		{
			...
			if(empty($iUserGroupId) && !Phpfox::getService('user.group.setting')->getGroupParam(Phpfox::getUserBy('user_group_id'), 'custom.has_special_custom_fields'))
			{
				$aCustomGroups = $this->database()->select('cg.group_id')
					->from(Phpfox::getT('cars_custom_group'), 'cg')
		...
		if (!isset($aItemData[$iItemId]))
		{
			...
			$sTable = Phpfox::getT('cars_custom');
			...
			foreach ($aFields as $sFieldName => $aRow)
			{
				...
				case 'select':
				    $aFields[$sFieldName]['value'] = $this->database()->select('co.phrase_var_name')
						    ->from(Phpfox::getT('cars_custom_option'), 'co')
						    ->join(Phpfox::getT('cars_custom_multiple_value'), 'cmv', 'cmv.option_id = co.option_id')
						    ->where('cmv.field_id = ' . (int)$aFields[$sFieldName]['field_id'] . ' AND cmv.car_id = ' . (int)$iItemId)
						    ->execute('getSlaveField');
				    break;
				case 'multiselect':
				case 'checkbox':
    ...
    /**
     * This function retrieves additional info to display in car.custom.info
     * @return array
     * @param type $iCarId
     */
    public function getUserPanelForUser($iCarId)
    {
        ...
        ->from($this->_sTable, 'cf')
        //->join(Phpfox::getT('module'), 'm', 'm.module_id = cf.module_id AND m.is_active = 1')
        ->join(Phpfox::getT('cars_custom_multiple_value'), 'cmv', 'cmv.field_id = cf.field_id')
                    ->join(Phpfox::getT('cars_custom_option'), 'co', 'co.option_id = cmv.option_id')
        ->where('cf.type_id = "cars_advanced_filter" AND cmv.car_id = ' . (int) $iCarId . '')
        ->order('cf.ordering ASC')
        ->execute('getSlaveRows');
        return $aFields;
    }
...
//			->from(Phpfox::getT('car'))
//			->where('car_id = ' . $iItemId)
//			->execute('getSlaveRow');
		...
		if ($aField['var_type'] == 'select' || $aField['var_type'] == 'radio')
		{
			$sUserValue = $this->database()->select('co.*')
				->from(Phpfox::getT('cars_custom_option'),'co')
				->join(Phpfox::getT('cars_custom_multiple_value'),'cmv','cmv.option_id = co.option_id')
				->where('cmv.car_id = ' . (int)$iItemId .' AND cmv.field_id = ' . (int)$iId)
				->execute('getSlaveRow');
		}
		...
		switch ($aField['var_type'])
		{
			...
			case 'multiselect':
				...
				$sContent .= '$.ajaxCall(\'custom.update\', \'field_id=' . $aField['field_id'] . '&amp;item_id=' . $iItemId . '&amp;edit_car_id=' . $iEditUserId . '&amp;\' + selectedNumbers);" /> - <a href="#" onclick="$(this).parents(\'.js_custom_content_holder:first\').find(\'.js_custom_content\').show(); $(this).parents(\'.js_custom_content_holder:first\').find(\'.js_custon_field\').hide(); return false;">' . Phpfox::getPhrase('custom.cancel') . '</a></div>';
				break;
			case 'checkbox':
				$aOptions = $this->database()->select('option_id, field_id, phrase_var_name')
					->from(Phpfox::getT('cars_custom_option'))
	...
	public function getForEdit($aTypes, $iItemId = null, $iUserGroup = null, $bRegister = false, $iCarId = null, $bToPrint=false)
	{
		$iGroup = 0;
		$sTable = 'cars_custom_multiple_value';
		$sTypes = '';
		...
		if($iCarId !== null)
		{
			$this->database()->leftjoin(Phpfox::getT('cars_custom_multiple_value'), 'cmv', 'cmv.field_id = cf.field_id AND cmv.car_id = ' . (int)$iCarId);
		}
		else
		{
			$this->database()->leftjoin(Phpfox::getT('cars_custom_option'), 'cmv', 'cmv.field_id = cf.field_id');
		}

		$aRows = $this->database()->select('cf.*, cmv.option_id as customValue, cg.user_group_id as cg_user_group_id')
			->from($this->_sTable, 'cf')
			->join(Phpfox::getT('module'), 'm', 'm.module_id = cf.module_id AND m.is_active = 1')
			->leftjoin(Phpfox::getT('cars_custom_group'), 'cg', 'cg.group_id = cf.group_id')
			->where($aWhere)
	...
	public function getForListing()
	{
		$aFields = $this->database()->select('cf.*, ug.title AS user_group_name')
            ...
            ->where('cf.product_id = \''.Phpfox::getLib('module')->getModuleName().'\'')
			->order('cf.ordering ASC')
			->execute('getRows');
		...
		$aGroups = $this->database()->select('cg.*, ug.title AS user_group_name')
            ...
            ->where('cg.product_id = \''.Phpfox::getLib('module')->getModuleName().'\'')
	...
	public function export($sProductId, $sModule = null)
	{
		...
		if (count($aRows))
		{
			$oXmlBuilder->addGroup('cars_custom_group');
