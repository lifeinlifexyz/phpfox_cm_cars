 ...
 */
class Cars_Service_Custom_Process extends Phpfox_Service
{	
...
//            var_dump($sCustomTable);
		...
		if ($this->database()->isField(($sCustomTable), Phpfox::getService('cars.custom')->getAlias() . $sFieldName))
		{
		...
		/*$this->database()->addField(array(
				'table' => ($sCustomTable),
				'field' => Phpfox::getService('cars.custom')->getAlias() . $sFieldName,
				'type' => $sTypeName
			)
		);*/
		switch($aVals['var_type'])
		...
		if ( $bAddField &&
			!$this->database()->isField(($sCustomValueTable), Phpfox::getService('cars.custom')->getAlias() . $sFieldName))
		{
			$this->database()->addField(array(
			...
			if ($sCustomValueTable != Phpfox::getT('cars_custom_value'))
			{
				$this->database()->addField(array(
						'table' => (Phpfox::getT('cars_custom_value')),
						'field' => Phpfox::getService('cars.custom')->getAlias() . $sFieldName,
						'type' => $sValueTypeName
					)
				);
		...
		if (!$this->database()->isField((Phpfox::getT('cars_custom')), Phpfox::getService('cars.custom')->getAlias() . $sFieldName))
		{
			$this->database()->addField(array(
	...
	public function updateFields($iItemId, $iEditUserId, $aVals, $bForce = false)
	{
		...
		$aChosen = $this->database()->select('*')
				->from(Phpfox::getT('cars_custom_multiple_value'))
				->where('car_id = ' . $iItemId . ' AND field_id IN (' . implode(',', array_map('intval', array_keys($aVals))) . ')')
				->execute('getSlaveRows');

		$aCarValues = $this->database()->select('*')
		...
		foreach ($aFields as $aField)
		{
			foreach ($aVals as $iFieldId => $aVal)
			{
				...
				{
					/* We check the cars_custom_value table */
					if (//empty($aVal) ||
						(
							isset($aCarValues[Phpfox::getService('cars.custom')->getAlias() . $aField['field_name']] )
							&& md5($aVal) == md5($aCarValues[Phpfox::getService('cars.custom')->getAlias() . $aField['field_name']])
						))
					{
						continue;
					}
	...
	public function updateField($aField, $iItemId, $iEditUserId, $sValue, $bForce = false)
	{
		...
		{
			$bCanEdit = false;
			if (($iEditUserId == Phpfox::getUserId() || Phpfox::isAdmin()/*&& Phpfox::getUserParam('custom.can_edit_own_custom_field')) || (Phpfox::getUserParam('custom.can_edit_other_custom_fields')*/))
			{
				$bCanEdit = true;
			}
		...
		$this->database()->delete(Phpfox::getT('cars_custom_multiple_value'), 'car_id = ' . $iItemId . ' AND field_id = ' . $aField['field_id']);
		...
		switch ($aField['var_type'])
		{
		 case 'radio':
				...
				$this->database()->insert(Phpfox::getT('cars_custom_multiple_value'), array(
					'car_id' => (int)$iItemId,
			...
			case 'checkbox':
				...
				if (isset($sOptionValue[0]['phrase_var_name']))
				{
					foreach ($sOptionValue as $iKey => $aVal)
					{
						...
						/* Add the option this user chose */
						$this->database()->insert(Phpfox::getT('cars_custom_multiple_value'), array('car_id' => $iItemId, 'field_id' => $aField['field_id'], 'option_id' => $aVal['option_id']));
					}
				}
			...
			case 'select':
				...
				{
					$this->database()->insert(Phpfox::getT('cars_custom_multiple_value'),
						array('car_id' => $iItemId,
			...
			case 'textarea':
				...
				if (!$this->database()->isField(Phpfox::getT($sTable), Phpfox::getService('cars.custom')->getAlias() . $aField['field_name']))
				{
					$this->database()->addField(array(
						'table' => Phpfox::getT($sTable),
						'field' => Phpfox::getService('cars.custom')->getAlias() . $aField['field_name'],
						'type' => $sTypeName
							)
					);
				}
				...
				if (!$this->database()->isField(Phpfox::getT($sValueTable), Phpfox::getService('cars.custom')->getAlias() . $aField['field_name']))
				{
					$this->database()->addField(array(
				...
				$aExisting = $this->database()->select('car_id as carCustom, ' . Phpfox::getService('cars.custom')->getAlias() . $aField['field_name']  . ' as currentValue')
				...
				if (isset($aExisting['carCustom']) && !empty($aExisting['carCustom']))
				{
					$this->database()->update(Phpfox::getT($sTable), array(
						Phpfox::getService('cars.custom')->getAlias() . $aField['field_name'] => $sInsertValue
					), 'car_id = ' . (int)$iItemId);
				}
				else
				{
					$this->database()->insert(Phpfox::getT($sTable),array('car_id' => (int)$iItemId, Phpfox::getService('cars.custom')->getAlias() . $aField['field_name'] => $sInsertValue));
				}

				$aExisting = $this->database()->select('car_id as carCustomValue')
						->from(Phpfox::getT($sValueTable))
						->where('car_id = ' . (int)$iItemId)
						->execute('getSlaveRow');

				if (isset($aExisting['carCustomValue']) && !empty($aExisting['carCustomValue']))
				{
				...
				{
					$this->database()->insert(Phpfox::getT($sValueTable),array('car_id' => (int)$iItemId, Phpfox::getService('cars.custom')->getAlias() . $aField['field_name'] => $sCleanValue));
				}
	...
	public function delete($iId)
	{
		...
		if ($aField['user_group_id'] > 0)
		{
			$aCustomFields = $this->database()->select('name, default_value')
					->from(Phpfox::getT('cars_group_custom'))
		...
		if ($this->database()->isField(Phpfox::getT($sTable), Phpfox::getService('cars.custom')->getAlias() . $aField['field_name']))
		{
			$this->database()->dropField(Phpfox::getT($sTable), Phpfox::getService('cars.custom')->getAlias() . $aField['field_name']);
		}

		if ($this->database()->isField(Phpfox::getT($sValueTable), Phpfox::getService('cars.custom')->getAlias() . $aField['field_name']))
		{
			$this->database()->dropField(Phpfox::getT($sValueTable), Phpfox::getService('cars.custom')->getAlias() . $aField['field_name']);
		}
		...
		//if ($aField['var_type'] == 'select')
		{
			...
			$this->database()->delete(Phpfox::getT('cars_custom_option'), 'field_id = ' . $aField['field_id']);
		}
		...
		{
		    $this->database()->delete(Phpfox::getT('cars_custom_multiple_value'),'field_id = ' . $aField['field_id']);
		}
	...
	public function deleteOption($iId)
	{
		...
		$aOption = $this->database()->select('co.*, cf.field_name, cf.user_group_id, cf.var_type, cf.field_id')
			...
			->join(Phpfox::getT('cars_custom_field'), 'cf', 'cf.field_id = co.field_id')
			...
			|| $aOption['var_type'] == 'checkbox' || $aOption['var_type'] == 'radio')
		{
		    ...
		    $this->database()->delete(Phpfox::getT('cars_custom_option'), 'option_id = ' . $aOption['option_id']);
		    $this->database()->delete(Phpfox::getT('cars_custom_multiple_value'), 'option_id = ' . $aOption['option_id'] . ' AND field_id = ' . $aOption['field_id']);
		    return true;
		}
		$sTable = 'cars_custom';
		$sValueTable = 'cars_custom_value';
		...
		$sFieldName = Phpfox::getService('cars.custom')->getAlias() . $aOption['field_name'];
	...
	{	    
		...
		foreach ($aVals['option'] as $iKey => $aOptions)
		{
		    ...
		    foreach ($aOptions as $sLang => $aOption)
		    {
				...
				{
					$this->_aOptions[$iKey . $sLang] = $this->database()->insert(Phpfox::getT('cars_custom_option'), array(
						'field_id' => $iFieldId,
