 ...
 * @version 		$Id: add.class.php 1121 2009-10-01 12:59:13Z Bolot_Kalil $
 */
class Cars_Component_Controller_Admincp_Custom_Group_Add extends Phpfox_Component
{
	/**
	...
	public function process()
	{
		...
		if (($iEditId = $this->request()->getInt('id')))
		{
			...
			if (($aGroup = Phpfox::getService('cars.custom.group')->getForEdit($iEditId)) && isset($aGroup['group_id']))
			{
				$bIsEdit = true;
...
//		}
		...
		if (($aVals = $this->request()->getArray('val')))
		{
			if ($oGroupValidator->isValid($aVals))
			{
				if ($bIsEdit === true)
				{
					if (Phpfox::getService('cars.custom.group.process')->update($aGroup['group_id'], $aVals))
					{
						$this->url()->send('admincp.cars.custom.group.add', array('id' => $aGroup['group_id']), Phpfox::getPhrase('custom.group_successfully_updated'));
					}
				}
				else 
				{
					if (Phpfox::getService('cars.custom.group.process')->add($aVals))
					{
						$this->url()->send('admincp.cars.custom.group.add', null, Phpfox::getPhrase('custom.group_successfully_added'));
					}
				}
			}
		}
