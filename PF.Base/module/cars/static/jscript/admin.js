		...
		switch (sAction)
		{
			...
			case 'delete':
				if (confirm(oTranslations['core.are_you_sure']))
				{
					...
					{
						$.ajaxCall('cars.deleteField', 'id=' + aParams['id']);
					}					
				}
				break;
			default:
				if (aParams['type'] == 'group')
				{
					$.ajaxCall('cars.toggleActiveGroup', 'id=' + aParams['id']);
				}
				else
				{
					$.ajaxCall('cars.toggleActiveField', 'id=' + aParams['id']);
				}				
				break;
		}
...
{	
	...
	$('.js_delete_current_option').click(function()
	{
		if (confirm(oTranslations['custom.are_you_sure_you_want_to_delete_this_custom_option']))
		{
			...
			$.ajaxCall('cars.deleteOption', 'id=' + aParams['id']);
		}
